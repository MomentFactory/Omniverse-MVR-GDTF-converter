...
TF_DECLARE_WEAK_AND_REF_PTRS(GdtfData);

/// \class GdtfSourceData
///
/// Serves as a wrapper around GdtfData for data providers to populate
/// information into.
///
class GdtfSourceData : public IGdtfSourceData
{
public:

	GdtfSourceData(GdtfData* data);
	virtual ~GdtfSourceData();
...
///
class GdtfData : public SdfAbstractData
{
public:

	static GdtfDataRefPtr CreateFromParameters(const GdtfDataParameters& parameters);
...
private:
	...
	friend class GdtfSourceData;

	// can only be constructed via CreateFromParameters
	GdtfData(std::unique_ptr<IGdtfDataProvider> dataProvider);
