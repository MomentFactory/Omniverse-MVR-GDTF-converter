...
#ifndef OMNI_GDTF_IGDTFDATAPROVIDER_H_
#define OMNI_GDTF_IGDTFDATAPROVIDER_H_
...
public:
	...
	static GdtfDataParameters FromFileFormatArgs(const SdfFileFormat::FileFormatArguments& args);
};

///
/// \class IGdtfSourceData
///
...
public:
	...
	GDTF_API virtual ~IGdtfSourceData();
	...
	///
	GDTF_API virtual void CreatePrim(const SdfPath& parentPath, const std::string& name, const SdfSpecifier& specifier,
	...
	///
	GDTF_API virtual void CreateAttribute(const SdfPath& parentPrimPath, const std::string& name, const SdfValueTypeName& typeName,
	...
	///
	GDTF_API virtual void SetField(const SdfPath& primPath, const TfToken& fieldName, const VtValue& value) = 0;
    ...
    /// 
    GDTF_API virtual bool HasField(const SdfPath& primPath, const TfToken& fieldName, VtValue* value) = 0;
	...
	///
	GDTF_API virtual bool HasAttribute(const SdfPath& attributePath, VtValue* defaultValue) = 0;
};

///
/// \class IGdtfDataProvider
///
...
{
public:
	GDTF_API virtual ~IGdtfDataProvider();

	// disallow copies
	IGdtfDataProvider(const IGdtfDataProvider&) = delete;
	IGdtfDataProvider& operator=(const IGdtfDataProvider&) = delete;
	...
	///
	GDTF_API virtual bool Read(std::shared_ptr<IGdtfSourceData> sourceData) = 0;
	...
	///                   they read data from their back-end.
	///
	GDTF_API virtual bool ReadChildren(const std::string& primPath, std::shared_ptr<IGdtfSourceData> sourceData) = 0;
	...
	GDTF_API virtual bool IsDataCached() const = 0;

protected:

	GDTF_API IGdtfDataProvider(const GdtfDataParameters& parameters);
	GDTF_API const GdtfDataParameters& GetParameters() const;

private:

	GdtfDataParameters _parameters;
};
