...
#include "gdtfData.h"
#include "gdtfDataProviderFactory.h"
#include "gdtfPluginManager.h"
...
GdtfDataParameters GdtfDataParameters::FromFileFormatArgs(const SdfFileFormat::FileFormatArguments& args)
{
	GdtfDataParameters parameters;
	parameters.dataProviderId = *(TfMapLookupPtr(args, GdtfDataParametersTokens->dataProviderId));
	
	// unpack the file format argument representation of the provider arguments
	std::string prefix = GdtfDataParametersTokens->providerArgs.GetString() + ":";
...
GdtfSourceData::GdtfSourceData(GdtfData* data)
{
	this->_data = data;
}

GdtfSourceData::~GdtfSourceData()
{
	this->_data = nullptr;
}

void GdtfSourceData::CreatePrim(const SdfPath& parentPath, const std::string& name, const SdfSpecifier& specifier,
...
void GdtfSourceData::CreateAttribute(const SdfPath& parentPrimPath, const std::string& name, const SdfValueTypeName& typeName,
...
void GdtfSourceData::SetField(const SdfPath& primPath, const TfToken& fieldName, const VtValue& value)
{
	if (this->_data != nullptr)
	{
...
bool GdtfSourceData::HasField(const SdfPath& primPath, const TfToken& fieldName, VtValue* value)
{
    if (this ->_data != nullptr)
    {
...
bool GdtfSourceData::HasAttribute(const SdfPath& attributePath, VtValue* defaultValue)
{
	if (this->_data != nullptr)
	{
...
GdtfData::GdtfData(std::unique_ptr<IGdtfDataProvider> dataProvider)
{
	this->_dataProvider = std::move(dataProvider);
	this->_sourceData = std::make_shared<GdtfSourceData>(this);
}

GdtfDataRefPtr GdtfData::CreateFromParameters(const GdtfDataParameters& parameters)
{
	std::unique_ptr<IGdtfDataProvider> dataProvider = GDTFPluginManager::GetInstance().CreateDataProvider(parameters.dataProviderId, parameters);
	if (dataProvider == nullptr)
	{
		// there was no provider responsible for this data or it didn't load properly,
		// so the best we can do is provide an empty GdtfData object with no backing provider
		// this will load nothing except an empty default Root prim
		return TfCreateRefPtr(new GdtfData(nullptr));
	}

	return TfCreateRefPtr(new GdtfData(std::move(dataProvider)));
}

void GdtfData::CreateSpec(const SdfPath& path, SdfSpecType specType)
{
...
void GdtfData::Erase(const SdfPath& path, const TfToken& fieldName)
{
	// not supported in this PoC
...
void GdtfData::EraseSpec(const SdfPath& path)
{
	// not supported in this PoC
...
VtValue GdtfData::Get(const SdfPath& path, const TfToken& fieldName) const
{
	VtValue val;
...
SdfSpecType GdtfData::GetSpecType(const SdfPath& path) const
{
...
bool GdtfData::Has(const SdfPath& path, const TfToken& fieldName, SdfAbstractDataValue* value) const
{
    if (value != nullptr)
	{
		VtValue val;
...
bool GdtfData::Has(const SdfPath& path, const TfToken& fieldName, VtValue* value) const
{
...
bool GdtfData::HasSpec(const SdfPath& path) const
{
	return this->GetSpecType(path) != SdfSpecType::SdfSpecTypeUnknown;
}

bool GdtfData::IsEmpty() const
{
	return false;
}

std::vector<TfToken> GdtfData::List(const SdfPath& path) const
{
	TfTokenVector names;
    SpecData::const_accessor accessor;
    if (_specData.find(accessor, path))
...
void GdtfData::MoveSpec(const SdfPath& oldPath, const SdfPath& newPath)
{
	// not supported in this PoC
...
void GdtfData::Set(const SdfPath& path, const TfToken& fieldName, const VtValue& value)
{
	// not supported in this PoC
...
void GdtfData::Set(const SdfPath& path, const TfToken& fieldName, const SdfAbstractDataConstValue& value)
{
	// not supported in this PoC
...
bool GdtfData::StreamsData() const
{
...
bool GdtfData::IsDetached() const
{
...
std::set<double> GdtfData::ListAllTimeSamples() const
{
	// not supported in this POC
	return std::set<double>();
}

std::set<double> GdtfData::ListTimeSamplesForPath(const SdfPath& path) const
{
	// not supported in this POC
	return std::set<double>();
}

bool GdtfData::GetBracketingTimeSamples(double time, double* tLower, double* tUpper) const
{
	// not supported in this POC
	return false;
}

size_t GdtfData::GetNumTimeSamplesForPath(const SdfPath& path) const
{
	// not supported in this POC
	return 0;
}

bool GdtfData::GetBracketingTimeSamplesForPath(const SdfPath& path, double time, double* tLower, double* tUpper) const
{
	// not supported in this POC
	return false;
}

bool GdtfData::QueryTimeSample(const SdfPath& path, double time, VtValue* optionalValue) const
{
	// not supported in this POC
	return false;
}

bool GdtfData::QueryTimeSample(const SdfPath& path, double time, SdfAbstractDataValue* optionalValue) const
{
	// not supported in this POC
	return false;
}

void GdtfData::SetTimeSample(const SdfPath& path, double time, const VtValue& value)
{
	// not supported in this POC
}

void GdtfData::EraseTimeSample(const SdfPath& path, double time)
{
	// not supported in this POC
}

void GdtfData::_VisitSpecs(SdfAbstractDataSpecVisitor* visitor) const
{
	// not supported in this POC
}
...
void GdtfData::_CreatePrim(const SdfPath& parentPath, const std::string& name, 
...
void GdtfData::_CreateAttribute(const SdfPath& primPath, const std::string& name,
...
void GdtfData::_CreateSpec(const SdfPath& path, const SdfSpecType& specType)
{
	SpecData::accessor accessor;
    if (_specData.find(accessor, path))
    {
...
bool GdtfData::_GetSpecTypeAndFieldValue(const SdfPath& path, 
...
bool GdtfData::_GetFieldValue(const SdfPath& path, 
...
void GdtfData::_SetFieldValue(const SdfPath& path, const TfToken& fieldName, const VtValue& value)
{
...
void GdtfData::_SetFieldValue(const SdfPath& path, const TfToken& fieldName, const VtValue& value) const
{
    // NOTE: if we ever wanted to add support for querying whether
    // the backend data provider could support writes, we should
    // query that here and ask them to write to their backing data store
