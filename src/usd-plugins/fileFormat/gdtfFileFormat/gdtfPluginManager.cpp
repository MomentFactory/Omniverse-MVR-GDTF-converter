...
TF_INSTANTIATE_SINGLETON(GDTFPluginManager);

TF_DEFINE_PRIVATE_TOKENS(
	GDTFDataProviderPlugInTokens,

	// metadata describing a unique id for the data provider plugin
	(dataProviderId)
);

GDTFPluginManager::GDTFPluginManager()
{
...
std::unique_ptr<IGdtfDataProvider> GDTFPluginManager::CreateDataProvider(const std::string& dataProviderId, const GdtfDataParameters& parameters)
{
	...
	std::unique_ptr<IGdtfDataProvider> dataProvider;
	GdtfDataProviderFactoryBase* factory = it->second.dataProviderType.GetFactory<GdtfDataProviderFactoryBase>();
	if (factory != nullptr)
	{
...
void GDTFPluginManager::_GetDataProviders()
{
	...
	if (!this->_pluginsLoaded)
	{
		...
		PlugRegistry::GetAllDerivedTypes(TfType::Find<IGdtfDataProvider>(), &dataProviderTypes);
		for (const TfType dataProviderType : dataProviderTypes)
		{
			...
			const JsOptionalValue dataProviderIdVal = JsFindValue(plugin->GetMetadataForType(dataProviderType), GDTFDataProviderPlugInTokens->dataProviderId.GetString());
			if (!dataProviderIdVal.has_value() || !dataProviderIdVal->Is<std::string>())
			{
				TF_CODING_ERROR("'%s' metadata for '%s' must be specified!", GDTFDataProviderPlugInTokens->dataProviderId.GetText(), dataProviderType.GetTypeName().c_str());
				continue;
			}
