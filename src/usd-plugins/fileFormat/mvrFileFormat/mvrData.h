...
#ifndef OMNI_MVR_MVRDATA_H_
#define OMNI_MVR_MVRDATA_H_
...
TF_DECLARE_WEAK_AND_REF_PTRS(MvrData);

/// \class MvrSourceData
///
/// Serves as a wrapper around MvrData for data providers to populate
/// information into.
///
class MvrSourceData : public IMvrSourceData
{
public:

	MvrSourceData(MvrData* data);
	virtual ~MvrSourceData();
...
/// a layer from files of "mvr" format.  This data is initialized
...
///
class MvrData : public SdfAbstractData
{
public:

	static MvrDataRefPtr CreateFromParameters(const MvrDataParameters& parameters);
...
private:
	...
	friend class MvrSourceData;

	// can only be constructed via CreateFromParameters
	MvrData(std::unique_ptr<IMvrDataProvider> dataProvider);
