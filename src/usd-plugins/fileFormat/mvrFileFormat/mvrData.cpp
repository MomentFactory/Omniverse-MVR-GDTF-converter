...
#include "mvrData.h"
#include "mvrDataProviderFactory.h"
#include "mvrPluginManager.h"
...
MvrDataParameters MvrDataParameters::FromFileFormatArgs(const SdfFileFormat::FileFormatArguments& args)
{
	MvrDataParameters parameters;
	parameters.dataProviderId = *(TfMapLookupPtr(args, MvrDataParametersTokens->dataProviderId));
	
	// unpack the file format argument representation of the provider arguments
	std::string prefix = MvrDataParametersTokens->providerArgs.GetString() + ":";
...
MvrSourceData::MvrSourceData(MvrData* data)
{
	this->_data = data;
}

MvrSourceData::~MvrSourceData()
{
	this->_data = nullptr;
}

void MvrSourceData::CreatePrim(const SdfPath& parentPath, const std::string& name, const SdfSpecifier& specifier,
...
void MvrSourceData::CreateAttribute(const SdfPath& parentPrimPath, const std::string& name, const SdfValueTypeName& typeName,
...
void MvrSourceData::SetField(const SdfPath& primPath, const TfToken& fieldName, const VtValue& value)
{
	if (this->_data != nullptr)
	{
...
bool MvrSourceData::HasField(const SdfPath& primPath, const TfToken& fieldName, VtValue* value)
{
    if (this ->_data != nullptr)
    {
...
bool MvrSourceData::HasAttribute(const SdfPath& attributePath, VtValue* defaultValue)
{
	if (this->_data != nullptr)
	{
...
MvrData::MvrData(std::unique_ptr<IMvrDataProvider> dataProvider)
{
	this->_dataProvider = std::move(dataProvider);
	this->_sourceData = std::make_shared<MvrSourceData>(this);
}

MvrDataRefPtr MvrData::CreateFromParameters(const MvrDataParameters& parameters)
{
	std::unique_ptr<IMvrDataProvider> dataProvider = MVRPluginManager::GetInstance().CreateDataProvider(parameters.dataProviderId, parameters);
	if (dataProvider == nullptr)
	{
		// there was no provider responsible for this data or it didn't load properly,
		// so the best we can do is provide an empty MvrData object with no backing provider
		// this will load nothing except an empty default Root prim
		return TfCreateRefPtr(new MvrData(nullptr));
	}

	return TfCreateRefPtr(new MvrData(std::move(dataProvider)));
}

void MvrData::CreateSpec(const SdfPath& path, SdfSpecType specType)
{
...
void MvrData::Erase(const SdfPath& path, const TfToken& fieldName)
{
	// not supported in this PoC
...
void MvrData::EraseSpec(const SdfPath& path)
{
	// not supported in this PoC
...
VtValue MvrData::Get(const SdfPath& path, const TfToken& fieldName) const
{
	VtValue val;
...
SdfSpecType MvrData::GetSpecType(const SdfPath& path) const
{
...
bool MvrData::Has(const SdfPath& path, const TfToken& fieldName, SdfAbstractDataValue* value) const
{
    if (value != nullptr)
	{
		VtValue val;
...
bool MvrData::Has(const SdfPath& path, const TfToken& fieldName, VtValue* value) const
{
...
bool MvrData::HasSpec(const SdfPath& path) const
{
	return this->GetSpecType(path) != SdfSpecType::SdfSpecTypeUnknown;
}

bool MvrData::IsEmpty() const
{
	return false;
}

std::vector<TfToken> MvrData::List(const SdfPath& path) const
{
	TfTokenVector names;
    SpecData::const_accessor accessor;
    if (_specData.find(accessor, path))
...
void MvrData::MoveSpec(const SdfPath& oldPath, const SdfPath& newPath)
{
	// not supported in this PoC
...
void MvrData::Set(const SdfPath& path, const TfToken& fieldName, const VtValue& value)
{
	// not supported in this PoC
...
void MvrData::Set(const SdfPath& path, const TfToken& fieldName, const SdfAbstractDataConstValue& value)
{
	// not supported in this PoC
...
bool MvrData::StreamsData() const
{
...
std::set<double> MvrData::ListAllTimeSamples() const
{
	// not supported in this POC
	return std::set<double>();
}

std::set<double> MvrData::ListTimeSamplesForPath(const SdfPath& path) const
{
	// not supported in this POC
	return std::set<double>();
}

bool MvrData::GetBracketingTimeSamples(double time, double* tLower, double* tUpper) const
{
	// not supported in this POC
	return false;
}

size_t MvrData::GetNumTimeSamplesForPath(const SdfPath& path) const
{
	// not supported in this POC
	return 0;
}

bool MvrData::GetBracketingTimeSamplesForPath(const SdfPath& path, double time, double* tLower, double* tUpper) const
{
	// not supported in this POC
	return false;
}

bool MvrData::QueryTimeSample(const SdfPath& path, double time, VtValue* optionalValue) const
{
	// not supported in this POC
	return false;
}

bool MvrData::QueryTimeSample(const SdfPath& path, double time, SdfAbstractDataValue* optionalValue) const
{
	// not supported in this POC
	return false;
}

void MvrData::SetTimeSample(const SdfPath& path, double time, const VtValue& value)
{
	// not supported in this POC
}

void MvrData::EraseTimeSample(const SdfPath& path, double time)
{
	// not supported in this POC
}

void MvrData::_VisitSpecs(SdfAbstractDataSpecVisitor* visitor) const
{
	// not supported in this POC
}
...
void MvrData::_CreatePrim(const SdfPath& parentPath, const std::string& name, 
...
void MvrData::_CreateAttribute(const SdfPath& primPath, const std::string& name,
...
void MvrData::_CreateSpec(const SdfPath& path, const SdfSpecType& specType)
{
	SpecData::accessor accessor;
    if (_specData.find(accessor, path))
    {
...
bool MvrData::_GetSpecTypeAndFieldValue(const SdfPath& path, 
...
bool MvrData::_GetFieldValue(const SdfPath& path, 
...
void MvrData::_SetFieldValue(const SdfPath& path, const TfToken& fieldName, const VtValue& value)
{
...
void MvrData::_SetFieldValue(const SdfPath& path, const TfToken& fieldName, const VtValue& value) const
{
    // NOTE: if we ever wanted to add support for querying whether
    // the backend data provider could support writes, we should
    // query that here and ask them to write to their backing data store
