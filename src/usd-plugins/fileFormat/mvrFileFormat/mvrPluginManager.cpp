...
TF_INSTANTIATE_SINGLETON(MVRPluginManager);

TF_DEFINE_PRIVATE_TOKENS(
	MVRDataProviderPlugInTokens,

	// metadata describing a unique id for the data provider plugin
	(dataProviderId)
);

MVRPluginManager::MVRPluginManager()
{
...
std::unique_ptr<IMvrDataProvider> MVRPluginManager::CreateDataProvider(const std::string& dataProviderId, const MvrDataParameters& parameters)
{
	...
	std::unique_ptr<IMvrDataProvider> dataProvider;
	MvrDataProviderFactoryBase* factory = it->second.dataProviderType.GetFactory<MvrDataProviderFactoryBase>();
	if (factory != nullptr)
	{
...
void MVRPluginManager::_GetDataProviders()
{
	...
	if (!this->_pluginsLoaded)
	{
		...
		PlugRegistry::GetAllDerivedTypes(TfType::Find<IMvrDataProvider>(), &dataProviderTypes);
		for (const TfType dataProviderType : dataProviderTypes)
		{
			...
			const JsOptionalValue dataProviderIdVal = JsFindValue(plugin->GetMetadataForType(dataProviderType), MVRDataProviderPlugInTokens->dataProviderId.GetString());
			if (!dataProviderIdVal.has_value() || !dataProviderIdVal->Is<std::string>())
			{
				TF_CODING_ERROR("'%s' metadata for '%s' must be specified!", MVRDataProviderPlugInTokens->dataProviderId.GetText(), dataProviderType.GetTypeName().c_str());
				continue;
			}
