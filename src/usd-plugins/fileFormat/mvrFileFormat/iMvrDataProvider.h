...
#ifndef OMNI_MVR_IMVRDATAPROVIDER_H_
#define OMNI_MVR_IMVRDATAPROVIDER_H_
...
public:
	...
	static MvrDataParameters FromFileFormatArgs(const SdfFileFormat::FileFormatArguments& args);
};

///
/// \class IMvrSourceData
///
...
public:
	...
	MVR_API virtual ~IMvrSourceData();
	...
	///
	MVR_API virtual void CreatePrim(const SdfPath& parentPath, const std::string& name, const SdfSpecifier& specifier,
	...
	///
	MVR_API virtual void CreateAttribute(const SdfPath& parentPrimPath, const std::string& name, const SdfValueTypeName& typeName,
	...
	///
	MVR_API virtual void SetField(const SdfPath& primPath, const TfToken& fieldName, const VtValue& value) = 0;
    ...
    /// 
    MVR_API virtual bool HasField(const SdfPath& primPath, const TfToken& fieldName, VtValue* value) = 0;
	...
	///
	MVR_API virtual bool HasAttribute(const SdfPath& attributePath, VtValue* defaultValue) = 0;
};

///
/// \class IMvrDataProvider
///
...
{
public:
	MVR_API virtual ~IMvrDataProvider();

	// disallow copies
	IMvrDataProvider(const IMvrDataProvider&) = delete;
	IMvrDataProvider& operator=(const IMvrDataProvider&) = delete;
	...
	///
	MVR_API virtual bool Read(std::shared_ptr<IMvrSourceData> sourceData) = 0;
	...
	///                   they read data from their back-end.
	///
	MVR_API virtual bool ReadChildren(const std::string& primPath, std::shared_ptr<IMvrSourceData> sourceData) = 0;
	...
	MVR_API virtual bool IsDataCached() const = 0;

protected:

	MVR_API IMvrDataProvider(const MvrDataParameters& parameters);
	MVR_API const MvrDataParameters& GetParameters() const;

private:

	MvrDataParameters _parameters;
};
